## Hi there 👋

I'm **Erfan** — a **Backend Developer** and **Blockchain Developer** 🚀  

---

- 🔭 I’m currently working on:  
  Building **scalable backend applications** with Laravel, and experimenting with **smart contracts** on EVM-compatible blockchains.  

- 🌱 I’m currently learning:  
  - Advanced **Solidity** for secure smart contract development  
  - **DeFi protocols** and yield farming strategies  
  - Improving **system design** and backend performance optimization  

- 👯 I’m looking to collaborate on:  
  - **Open-source blockchain projects** (DeFi, NFTs, DEXs)  
  - **Backend systems** that need scalability and reliability  

- 💬 Ask me about:  
  - Backend development with **Laravel**, and **RESTful APIs**  
  - Writing and auditing **smart contracts** in **Solidity**  
  - Designing efficient **database structures** and **microservices**  

- 📫 How to reach me:  
  - GitHub: [@erfantq](https://github.com/erfantq)  
  - LinkedIn: https://www.linkedin.com/in/erfan-taghavi-5219432b3/
  - Email: [erfantaghavi319@gmail.com](mailto:erfantaghavi319@gmail.com)
  - Telegram: https://t.me/erfaaan_tq

- ⚡ Fun fact:  
  I enjoy exploring how **decentralized systems** can solve real-world problems, and sometimes I lose track of time deep-diving into new tech 🌌  

---
✨ Always open to **collaborations, discussions, and learning new things**!  
